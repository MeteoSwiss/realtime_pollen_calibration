...
3. Create a virtual environment and install the development dependencies:
    ...
    This will create a conda environment named `realtime-pollen-calibration-dev` (change with `-n`) and install the following:
    ...
    ```bash
    conda activate realtime-pollen-calibration-dev
    ```
...
Realtime Pollen Calibration uses [Conda](https://docs.conda.io/en/latest/) to manage dependencies. (Also check out [Mamba](https://mamba.readthedocs.io/en/latest/) if you like your package installations fast.) Dependencies are specified in YAML files, of which there are four:
